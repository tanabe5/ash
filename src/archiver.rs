    ...
    match format {
        Ok(format) => {
            return match format {
                ...
                _ => Err(AshError::UnknownFormat(format.to_string())),
            }
        }
        Err(msg) => Err(msg),
    }
...
impl ArchiverOpts {
    ...
    pub fn destination(&self) -> Result<File> {
        ...
        if p.is_file() && p.exists() && !self.overwrite {
            return Err(AshError::FileExists(self.dest.clone()));
        }
        if let Some(parent) = p.parent() {
            if !parent.exists() {
                if let Err(e) = create_dir_all(parent) {
                    return Err(AshError::IO(e));
                }
            }
        }
        match File::create(self.dest.as_path()) {
            Ok(f) => Ok(f),
            Err(e) => Err(AshError::IO(e)),
        }
    }
}
...
mod tests {
    ...
    fn test_archiver() {
        ...
        assert!(a10.is_err());
        if let Err(e) = a10 {
            if let AshError::UnknownFormat(msg) = e {
