...
fn perform_list(opts: CliOpts) -> Result<()> {
    ...
    for arg in args.iter() {
        if !arg.exists() {
            return Err(AshError::FileNotFound(arg.to_path_buf()));
        }
        let extractor = extractor::create_extractor(&arg).unwrap();
        if args.len() > 1 {
            println!("========== {:?} ========== \n", arg);
...
fn main() -> Result<()> {
    match perform(CliOpts::parse()) {
        ...
        Err(e) => {
            match e {
                AshError::NoArgumentsGiven => {
                    println!("No arguments given. Use --help for usage.")
                }
                AshError::FileNotFound(p) => println!("{}: file not found", p.to_str().unwrap()),
                AshError::FileExists(p) => {
                    println!("{}: file already exists", p.to_str().unwrap())
                }
                AshError::IO(e) => println!("IO error: {}", e),
                AshError::Archiver(s) => println!("Archive error: {}", s),
                AshError::UnknownFormat(f) => println!("{}: unknown format", f),
                AshError::UnsupportedFormat(f) => println!("{}: unsupported format", f),
                AshError::Fatal(e) => println!("Error: {}", e),
                AshError::Unknown(s) => println!("Unknown error: {}", s),
            }
            std::process::exit(1);
        }
    }
